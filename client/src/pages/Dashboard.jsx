...
import { Image, Video, Smile, Upload, PlusCircle, Sparkles, BookOpen, Heart } from "lucide-react";
...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      {/* Welcome Board */}
      <div className="bg-[var(--bg-card)] border-3 border-[var(--border)] rounded-xl p-6 shadow-[5px_5px_0px_0px_var(--shadow-color)] flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden">
        <div className="absolute right-[-20px] top-[-20px] opacity-10 pointer-events-none heartbeat">
          <Heart size={160} className="text-[var(--color-primary)]" fill="var(--color-secondary)" />
        </div>
            ...
            {recentMedia.map((item) => (
              ...
              >
                ...
                <div className="h-40 bg-[var(--bg-primary)] border-b-2 border-[var(--border)] overflow-hidden relative">
                  ...
                  <span className="absolute top-2 right-2 badge badge-warning text-[8px] font-black tracking-widest shadow-[1px_1px_0px_0px_var(--shadow-color)] py-0.5 px-2">
                    {item.mediaType}
                  </span>
                </div>
                ...
                <div className="p-3 flex-1 flex flex-col justify-between space-y-2">
                  ...
                  {item.story ? (
                    <p className="text-[10px] font-medium text-[var(--text-primary)] line-clamp-2 bg-[var(--bg-primary)] p-1.5 rounded border border-[var(--border)] italic">
                      "{item.story}"
                    </p>
                  ) : (
                    <div className="text-[9px] font-extrabold text-[var(--color-primary)] uppercase flex items-center gap-1 bg-[var(--bg-primary)] border border-[var(--border)] px-1.5 py-0.5 rounded w-max">
                      No description yet
                    </div>
                  )}
                </div>
          ...
          {quickActions.map((act) => {
            ...
            return (
              <Link
                key={act.name}
                to={act.path}
                className="flex flex-col justify-between p-4 bg-[var(--bg-card)] border-2 border-[var(--border)] rounded-lg shadow-[3px_3px_0px_0px_var(--shadow-color)] hover:scale-102 hover:shadow-[4px_4px_0px_0px_var(--color-secondary)] hover:-translate-y-0.5 no-underline transition-all text-left h-44"
              >
                <div
                  className="w-10 h-10 rounded-lg border-2 border-[var(--border)] flex items-center justify-center shadow-[1.5px_1.5px_0px_0px_var(--shadow-color)]"
                  style={{ backgroundColor: act.bg }}
                >
                  <Icon size={18} color="white" strokeWidth={2.5} />
                </div>
                <div className="mt-2 space-y-1">
                  <div className="text-xs font-black text-[var(--text-primary)] uppercase">{act.name}</div>
                  <div className="text-[9px] font-semibold text-[var(--text-secondary)] leading-relaxed uppercase">{act.text}</div>
                </div>
                <div className="text-[10px] font-extrabold text-[var(--color-primary)] uppercase mt-2 flex items-center gap-1">
                  Add <PlusCircle size={10} strokeWidth={2.5} />
                </div>
              </Link>
            );
          })}
